...
exports.sourceNodes = async (
  ...
  { apiURL = 'http://localhost:1337', contentTypes = [], singleTypes = [], graphqlTypes = [], loginData = {}, queryLimit = 100 }
) => {
  ...
  // Generate a list of promises based on the `singleTypes` option.
  const graphqlTypePromises = graphqlTypes.map(graphqlType =>
    fetchData({
      apiURL,
      graphqlType,
      jwtToken,
  ...
  let entities = await Promise.all([...contentTypePromises, ...singleTypePromises, ...graphqlTypePromises])
  ...
  contentTypes.concat(singleTypes).concat(graphqlTypes).forEach((contentType, i) => {
    const items = entities[i]
    items.forEach(item => {
      let type = typeof contentType === 'string' ? contentType : contentType.type
